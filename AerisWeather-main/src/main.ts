...
async function init() {
  ...
  map.addLayer({
    ...
    paint: {
      ...
      'raster-saturation': -0.8,   // slight desaturation — keeps subtle colour for cloud-type hints
  ...
  createUI(uiContainer, weather, {
    ...
    onLayerToggle: (layer, active) => {
      ...
      if (layer === 'clouds') {
        ...
        // Also toggle the satellite texture overlay — but only when in global view
        if (!currentCity) setCloudOverlayVisible(active);
      }
