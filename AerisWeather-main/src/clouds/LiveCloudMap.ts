...
export interface CoverageMap {
  data: Float32Array;       // blended (satellite + noise) — used by CloudPointLayer global view
  raw: Float32Array;        // pure satellite brightness per cell — used by volumetric city clouds
  redness: Float32Array;    // R channel — warm tones hint at low/thick clouds
  blueness: Float32Array;   // B channel — cool/thin tones hint at high cirrus
  width: number;
  height: number;
  timestamp: number;
  source: string;
...
interface RawCloudData {
  brightness: Float32Array;  // 0-1, grayscale cloud density
  redness: Float32Array;     // R channel normalized — warm tones = low cloud
  blueness: Float32Array;    // B channel normalized — cool tones = high/thin cloud
}

/**
 * Fetch the live cloud map and extract pixel data.
 */
async function fetchCloudMap(): Promise<RawCloudData | null> {
  return new Promise((resolve) => {
    ...
    img.onload = () => {
      ...
      try {
        ...
        const brightness = new Float32Array(GRID_W * GRID_H);
        const redness    = new Float32Array(GRID_W * GRID_H);
        const blueness   = new Float32Array(GRID_W * GRID_H);
        ...
        for (let j = 0; j < GRID_H; j++) {
          for (let i = 0; i < GRID_W; i++) {
            ...
            // Greyscale brightness: bright = cloudy, dark = clear sky
            const br = (r + g + b) / (3 * 255);
            brightness[j * GRID_W + i] = br;

            // Colour channels for cloud-type discrimination
            // The matteason cloud map is mostly greyscale, but subtle tones exist
            redness[j * GRID_W + i]  = r / 255;
            blueness[j * GRID_W + i] = b / 255;
          }
        }

        resolve({ brightness, redness, blueness });
      } catch (e) {
...
export class LiveCloudMap {
  ...
  async refresh(): Promise<CoverageMap | null> {
    ...
    const coverage = blendWithNoise(raw.brightness, this.noise);
