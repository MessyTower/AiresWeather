...
import type { CoverageMap } from '../clouds/LiveCloudMap';
...
export class DeckLayers {
  ...
  // Satellite cloud map — from LiveCloudMap (used as city cloud seed)
  private satelliteCoverage: CoverageMap | null = null;
  ...
  setSatelliteCoverage(map: CoverageMap): void {
    this.satelliteCoverage = map;
    // Regenerate city clouds with the new satellite data
    if (this.focusedCity) {
      this.generateVolumetricClouds(this.focusedCity);
      this.updateLayers();
    }
  }
  ...
  private generateVolumetricClouds(city: City): void {
    const sat = this.satelliteCoverage;

    // ── Helper: sample satellite map at geographic coordinate ──────────
    const sampleSat = (lon: number, lat: number, field: Float32Array): number => {
      if (!sat) return 0;
      const w = sat.width, h = sat.height;
      // Map lon -180..180 → 0..w,  lat 90..-90 → 0..h
      const ix = Math.floor(((lon + 180) / 360) * w) % w;
      const iy = Math.max(0, Math.min(h - 1, Math.floor(((90 - lat) / 180) * h)));
      return field[iy * w + ix];
    };

    // ── Check satellite coverage at city centre ───────────────────────
    // Use a small averaging window (±1°) for robustness
    let satCoverage = 0;
    if (sat) {
      let sum = 0, count = 0;
      for (let dlat = -1; dlat <= 1; dlat++) {
        for (let dlon = -1; dlon <= 1; dlon++) {
          sum += sampleSat(city.lon + dlon, city.lat + dlat, sat.raw);
          count++;
        }
      }
      satCoverage = sum / count;
    }

    // If satellite shows clear sky, skip generation entirely
    if (sat && satCoverage < 0.08) {
      console.log(`[VolumetricClouds] Clear sky at ${city.name} (satellite=${satCoverage.toFixed(3)}) — skipping`);
      this.volumetricClouds = [];
      return;
    }

    // Fall back to Open-Meteo fraction if satellite not yet loaded
    if (!sat) {
      ...
      if (grid?.fields.cloudFraction) {
        ...
        const meteoFraction = grid.fields.cloudFraction[idx] ?? 0;
        if (meteoFraction < 0.02) {
          this.volumetricClouds = [];
          return;
        }
      }
    ...
    console.log(`[VolumetricClouds] ${city.name}: satellite=${satCoverage.toFixed(3)} — generating`);

    // ── Noise maps for detail shaping (NOT for cloud presence!) ───────
    ...
    for (let dy = -regionRadius; dy <= regionRadius; dy += gridStep) {
      for (let dx = -regionRadius; dx <= regionRadius; dx += gridStep) {
        const sampleLon = city.lon + dx;
        const sampleLat = city.lat + dy;

        // ── Satellite is the GATE: if no cloud here, skip completely ──
        const satBright = sat ? sampleSat(sampleLon, sampleLat, sat.raw) : satCoverage;
        if (satBright < 0.08) continue;

        // Cloud-type hints from satellite colour channels
        // Bright white (high R,G,B) = thick low/mid cloud or CB
        // Slightly blue-shifted = high thin cirrus
        const satBlue = sat ? sampleSat(sampleLon, sampleLat, sat.blueness) : 0.5;
        const satRed  = sat ? sampleSat(sampleLon, sampleLat, sat.redness)  : 0.5;
        // blueRatio > 1 means cooler (higher) cloud; < 1 means warmer (lower) cloud
        const blueRatio = satBright > 0.05 ? (satBlue / Math.max(0.01, satRed)) : 1.0;
        const isHighCloud = blueRatio > 1.04;  // subtle blue shift → cirrus/alto
        const isThickLow  = satBright > 0.55 && blueRatio < 1.0;

        // ── Noise provides SHAPE, not existence ───────────────────────
        ...
        // Satellite brightness drives overall density; noise adds internal variation
        // noise is used as 0.6..1.0 multiplier (detail) not as a threshold
        const noiseDetail = 0.6 + noiseVal * 0.4;
        const cloudDensity = satBright * edgeFade * noiseDetail;
        if (cloudDensity < 0.06) continue;

        // Height bands: satellite colour hints which bands are active
        // isHighCloud → prefer upper bands; isThickLow → all bands; otherwise low+mid
        let maxBand: number;
        if (isHighCloud) {
          maxBand = bands.length;                         // include cirrus bands
        } else if (isThickLow) {
          maxBand = Math.ceil((0.5 + hVal * 0.5) * bands.length);
        } else {
          maxBand = Math.ceil((0.3 + hVal * 0.4) * bands.length);  // mostly low/mid
        }

        for (let bi = 0; bi < Math.min(maxBand, bands.length); bi++) {
          const cfg = bands[bi];

          // Skip high bands for clearly low/warm clouds
          if ((cfg.type === 'cirrus' || cfg.type === 'stratus') && !isHighCloud && satBright < 0.35) continue;
          if (cfg.type === 'anvil' && stormVal < 0.2) continue;

          let eff = cloudDensity;
          if (cfg.type === 'anvil')  eff *= stormVal * 1.5;
          if (cfg.type === 'cirrus') eff *= isHighCloud ? 1.2 : 0.4;
    ...
    console.log(`[VolumetricClouds] ${points.length} pts for ${city.name} (satellite-seeded)`);
  }
